...
namespace MusicPlayer
{
    class ComponentPositionEditor : System.Drawing.Design.UITypeEditor
    {
        ...
        protected virtual void SetEditorProps(MusicPlayer editingInstance, CompEditPanel editor)
        {
            ...
            //for (int i = 1; i < 9; i++) {
            //    editingInstance.component_num = i;
            //    editor.setComponentsRectsPosition(i, (int)(editingInstance.ComponentRect[i-1].X * (float)editor.Width / (float)editingInstance.Width), (int)(editingInstance.ComponentRect[i-1].Y * (float)editor.Height / (float)editingInstance.Height));
            //    editor.setComponentRectsSize(i, (int)(editingInstance.ComponentRect[i-1].Width * (float)editor.Width / (float)editingInstance.Width), (int)(editingInstance.ComponentRect[i-1].Height * (float)editor.Height / (float)editingInstance.Height));
            //}
        }
        int i = 0;
        public override object EditValue(ITypeDescriptorContext context, IServiceProvider provider, object value)
        {
                ...
                && provider != null)
            {
                ...
                if (edSvc != null)
                {
                    //   LayerEditPanel pan = new LayerEditPanel((context.Instance as BasePlayer).locatio, (context.Instance as BasePlayer).CompNameList);
                    //    pan.Theme = (context.Instance as BasePlayer).Theme;
                 ...
                 //   if (pan.Element_color != Color.Transparent)
                //    {
